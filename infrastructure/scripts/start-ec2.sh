#!/usr/bin/env bash
# Start docker compose services on EC2 without recreating containers/volumes.
# Usage:
#   ./infrastructure/scripts/start-ec2.sh
#   ./infrastructure/scripts/start-ec2.sh web app
set -euo pipefail
...
if [[ ${#SERVICES[@]} -eq 0 ]]; then
  REMOTE_CMD='cd /opt/dentis && sudo docker compose start'
  echo "[INFO] Starting all compose services on EC2 ($INSTANCE_IP)..."
else
  REMOTE_CMD="cd /opt/dentis && sudo docker compose start ${SERVICES[*]}"
  echo "[INFO] Starting services on EC2 ($INSTANCE_IP): ${SERVICES[*]}"
fi

ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no ec2-user@"$INSTANCE_IP" "$REMOTE_CMD"

echo "[OK] Services started."
echo "[INFO] If a service was never created before, use deploy.sh (or docker compose up -d) once."
