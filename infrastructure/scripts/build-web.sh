#!/usr/bin/env bash
# 2. Build frontend Docker image, push to ECR and restart web on EC2.
#    If no EC2 exists yet, just pushes the image to ECR.
#    Usage: ./infrastructure/scripts/build-web.sh
set -euo pipefail
...
REPO_NAME="${APP_NAME}-${ENVIRONMENT}-web"
...
echo "[INFO] Building web image: ${REPO_URL}:${WEB_IMAGE_TAG}"
docker buildx build --platform "$DOCKER_PLATFORMS" \
  -f "$ROOT_DIR/infrastructure/docker/Dockerfile.web" \
  -t "${REPO_URL}:${WEB_IMAGE_TAG}" --push "$ROOT_DIR"
echo "[OK] Web image pushed to ECR"
...
echo "[INFO] Restarting web on EC2 ($INSTANCE_IP)..."
...
echo "[OK] Web restarted."
echo "     Status: $(curl -s -o /dev/null -w '%{http_code}' "http://$INSTANCE_IP:${WEB_PORT}/" 2>/dev/null || echo 'unreachable')"
