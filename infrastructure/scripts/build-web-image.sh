...
echo "[INFO] Image Tag: $WEB_IMAGE_TAG"
...
docker buildx build \
  ...
  -t "${WEB_REPO_URL}:${WEB_IMAGE_TAG}" \
  --push \
  "$ROOT_DIR"


echo "[INFO] ✓ Web image built and pushed successfully"
echo "[INFO] Image URL: ${WEB_REPO_URL}:${WEB_IMAGE_TAG}"
